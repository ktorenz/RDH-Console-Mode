...
namespace gcmloader
{
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        private const string GithubLatestReleaseApiUrl = "https://api.github.com/repos/ktorenz/RDH-Console-Mode/releases/latest";
...
private static readonly string SettingsFilePath = Path.Combine(SettingsFolder, "settings.toml");
        ...
        public MainWindow()
        {
           // Logger.Initialize();
            ...
            // RDH patch: the tag hardcodes a Steam logo onto the black boot overlay.
            // In Playnite direct-launch mode the overlay stays visible until Playnite
            // covers it (we defer TransitionToMainUI), so the logo would sit on screen
            // for the whole boot. Keep it pure black instead - console-style boot,
            // handing off to Playnite's own intro video.
            if (RdhDirectPlayniteMode())
            {
                if (FocusLossOverlay.Child is Image blankLogo)
                {
                    blankLogo.Source = null;
                }
            }
            else
            {
        ...
        private static readonly List<(string ServiceName, string? ProcessName)> _debloatServicesHandheld = new()
{
    ...
    ("ContactDataSvc", null),
        ...
        {
            // RDH patch: hard stop for every Steam launch path in Playnite mode.
            // Individual callers are mostly unreachable already, but this closes
            // all of them at once.
            if (RdhDirectPlayniteMode())
            {
                App.StartupTrace("RDH: StartSteam suppressed in Playnite mode.");
                return;
            }
        ...
        private async Task Start()
        {
            try
            {
                ...
                // --- RDH patch: in Playnite mode, launch Playnite directly with the GCM UI
                // still hidden, then reveal the UI behind it (see DeferredUiRevealAsync).
                // The v2.6.8 tag's StartConfiguredLauncherAsync is Steam-only, so Playnite
                // mode routes to the existing (previously uncalled) StartPlaynite().
                if (RdhDirectPlayniteMode())
                {
                    App.StartupTrace("RDH direct launch: starting Playnite with GCM UI hidden.");
                    await StartPlaynite();
...
-----END PUBLIC KEY-----";
        ...
        // Resolves a card image: absolute path as-is, bare filename relative to
        // %APPDATA%\gcmsettings. Returns null when the file does not exist, so
        // callers can fall back cleanly.
        private static string RdhResolveCardImage(string name)
        {
            if (string.IsNullOrWhiteSpace(name)) return null;
            string p = Path.IsPathRooted(name) ? name : Path.Combine(RdhSettingsDir, name);
            return File.Exists(p) ? p : null;
        }

        private sealed class RdhFixedCard
        {
            public string name { get; set; }
        ...
        private void RdhAddFixedLauncherCards()
        {
            ...
            {
                App.StartupTrace($"RDH fixed cards failed: {ex.Message}");
            }
        }

        private async Task RdhLaunchFixedAppAsync(string exePath, string exeArgs)
        {
            try
            {
                var running = Process.GetProcessesByName(Path.GetFileNameWithoutExtension(exePath))
                    .FirstOrDefault(p => p.MainWindowHandle != IntPtr.Zero);
                if (running != null)
                {
                    MakeSelfNonTopmost();
                    await ForcefullyBringToForeground(running.MainWindowHandle);
                    return;
                }
                StartProcessAsNonAdmin(exePath,
                    string.IsNullOrWhiteSpace(exeArgs) ? null : exeArgs,
                    Path.GetDirectoryName(exePath));
            }
            catch (Exception ex)
            {
                App.StartupTrace($"RDH fixed card launch failed: {ex.Message}");
            }
        }

        // Direct-launch mode: boot goes straight to Playnite Fullscreen without
        // ever showing the GCM UI. GCM keeps running hidden as the shell, and the
        // UI is revealed BEHIND Playnite once its window exists, so quitting
        ...
        private async Task DeferredUiRevealAsync()
        {
            try
            {
                // Wait for Playnite's window (up to ~20s). If it never appears,
                // fall through and reveal the UI anyway so the user is not
                // stranded on a black screen.
                Process playnite = null;
                for (int i = 0; i < 100; i++)
                {
                    playnite = Process.GetProcessesByName("Playnite.FullscreenApp").FirstOrDefault();
                    if (playnite != null && playnite.MainWindowHandle != IntPtr.Zero) break;
                    await Task.Delay(200);
                }

                App.StartupTrace("RDH direct launch: revealing GCM UI behind launcher.");
        ...
        private void LoadDynamicLauncherCards()
        {
            ...
            // RDH patch: the tag hardcodes the primary launcher card to Steam
            // (name, logo, and a tap action that is Steam top to bottom). In
            // Playnite mode, build a Playnite card instead. StartPlaynite()
            // already has taskbar-card semantics: foreground if running,
            // launch if not.
            bool rdhPlayniteCard = RdhDirectPlayniteMode();
            // Icon override: drop launchercard_icon.png into %APPDATA%\gcmsettings
            string mainLauncherIconPath = rdhPlayniteCard
                ? (RdhResolveCardImage("launchercard_icon.png") ?? "ms-appx:///Assets/playnite_logo.png")
                : "ms-appx:///Assets/steam_logo.png";

            // 1. MAIN LAUNCHER (Wird groß erstellt: 250x250)
            var mainLauncherItem = new LauncherCardItem
            {
                Name = rdhPlayniteCard ? "Playnite" : "Steam",
                Subtitle = "Launcher",
                Description = rdhPlayniteCard
                    ? "Return to Playnite Fullscreen."
                    : "Zuruck in Steam Big Picture und direkt wieder in die Hauptoberflache springen.",
                ImagePath = mainLauncherIconPath,
                IsPrimary = true,
                TapAction = rdhPlayniteCard
                    ? (s, e) => _ = StartPlaynite()
                    : (Action<object, Microsoft.UI.Xaml.Input.TappedRoutedEventArgs>)((s, e) => SwitchToConfiguredLauncher())
            };
            var mainCard = CreateLauncherCard(mainLauncherItem);
            LauncherAreaPanel.Children.Add(mainCard);
            _launcherAreaButtons.Add(mainCard);

            // RDH patch: in Playnite mode, replace the running-app ("recent") card
            // with the fixed cards defined in rdh_cards.json.
            if (rdhPlayniteCard)
            {
                RdhAddFixedLauncherCards();
            }
            else if (_featuredGameProcessData?.GameInfo?.IsGame == true &&
        ...
        private Border CreateLauncherCard(LauncherCardItem item)
        {
            ...
            // RDH patch: in Playnite mode the main card's background art comes from
            // %APPDATA%\gcmsettings\launchercard.png (or .jpg) when present, else
            // the card's glass look. Steam mode keeps the stock Steam artwork.
            BitmapImage launcherBackgroundSource = null;
            if (isMainLauncher)
            {
                if (RdhDirectPlayniteMode())
                {
                    string rdhBg = RdhResolveCardImage("launchercard.png")
                                   ?? RdhResolveCardImage("launchercard.jpg");
                    if (rdhBg != null) launcherBackgroundSource = new BitmapImage(new Uri(rdhBg));
                }
                else
                {
                    launcherBackgroundSource = new BitmapImage(new Uri("ms-appx:///Assets/steam_launcher_background.jpg"));
                }
            }
...
        private FocusArea _previousFocusArea = FocusArea.Cards; // Used to track which area to return to from TopButtons
        ...
        {
            // RDH patch: Steam startup-video injection never applies in Playnite mode.
            if (RdhDirectPlayniteMode()) return false;
        ...
        private void PlayStartupVideo()
        {
            try
            {
                ...
                if (!IsGcmVideoEnabled() || IsSteamInjectionEnabled())
                {
                    ...
                    if (RdhDirectPlayniteMode())
                    {
                        // RDH patch: unblock Start()'s wait loop without showing the
                        // UI. The reveal happens later, behind Playnite.
                        App.StartupTrace("RDH direct launch: UI reveal deferred.");
                ...
                if (string.IsNullOrEmpty(videoPath) || !File.Exists(videoPath))
                {
                    ...
                    if (RdhDirectPlayniteMode())
                    {
                        // RDH patch: same deferral as the skip branch above.
                        App.StartupTrace("RDH direct launch: UI reveal deferred.");
                        startupVideoFinished = true;
                    }
                    else
        ...
        // RDH patch: `force` lets the deferred reveal run even after the direct-launch
        // path has already set startupVideoFinished to unblock Start(). All original
        // callers pass nothing and behave exactly as before.
        private void TransitionToMainUI(bool force = false)
        {
            if ((startupVideoFinished && !force) || _isTransitioningToMainUi) return; // Verhindert doppeltes Ausführen
