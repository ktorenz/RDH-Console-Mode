...
namespace gcmloader
{
    public sealed partial class MainWindow
    {
        ...
        // RDH patch: in Playnite mode every Steam integration is hard-off,
        // regardless of stored settings - no store sync, no plugin host loop.
        private bool IsSteamStoreSyncEnabled() => !RdhDirectPlayniteMode() && GetSetting(SteamStoreSyncEnabledKey, true);

        private bool IsSteamPluginHostEnabled() => !RdhDirectPlayniteMode() && GetSetting(SteamPluginHostEnabledKey, true);
        ...
        {
            // RDH patch: never start the Steam plugin host loop in Playnite mode,
            // even when a caller skips the IsSteamPluginHostEnabled() check.
            if (RdhDirectPlayniteMode())
            {
                App.StartupTrace("RDH: Steam plugin host suppressed in Playnite mode.");
                return;
            }
